           ...
           boton1.Clicked += (sender, e) =>
{
    ...
    else
    {
        ...
        ListaUsuarios listaUsuarios = ListaUsuarios.ObtenerInstancia();
        Usuario usuarioEncontrado = listaUsuarios.ObtenerUsuarioPorCorreo(correo);
        RegistrarEntrada(correo); 
        if (usuarioEncontrado != null && usuarioEncontrado.Contrasenia == contrasenia)
        {
            Console.WriteLine($"Inicio de sesión exitoso como usuario: {usuarioEncontrado.Nombres}");
     ...
     private void RegistrarEntrada(string correo)
    {
        string horaEntrada = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        string log = $"{{ \"usuario\": \"{correo}\", \"entrada\": \"{horaEntrada}\" }}";
