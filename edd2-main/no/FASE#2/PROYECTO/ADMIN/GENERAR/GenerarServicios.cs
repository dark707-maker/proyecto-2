...
class GenerarServicios : Gtk.Window {
    ...
    try {
        ...
        int idServicio = int.Parse(entryId.Text);
        ...
        // Obtener el ID del usuario asociado al vehículo
        int idUsuario = vehiculo.IdUsuario;

        // Validar si el ID del repuesto existe
        var repuesto = arbolRepuestos.BuscarPorId(idRepuesto);
        if (repuesto == null) {
            Console.WriteLine($"Error: El ID del repuesto {idRepuesto} no existe.");
            return;
        }

        // Agregar el servicio al árbol de servicios
        arbolServicios.Agregar(idServicio, idRepuesto, idVehiculo, detalles, costo, idUsuario);

        // Generar y almacenar la factura en el árbol B
        arbolFacturas.Insertar(idServicio, idUsuario, idServicio, costo);
