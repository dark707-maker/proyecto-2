...
class ArbolServicios {
    ...
    private static int contadorFacturas = 1; // Contador global para las facturas
    ...
    public void Agregar(int id, int idRepuesto, int idVehiculo, string detalles, double costo, int idUsuario)
{
    NodoServicio nuevoServicio = new NodoServicio(id, idRepuesto, idVehiculo, detalles, costo);
    ...
    {
        AgregarRecursivo(root, nuevoServicio);
    }

    // Generar factura automáticamente para este servicio
    GenerarFacturaAutomatica(id, idVehiculo, costo, idRepuesto); // Asegúrate de pasar idRepuesto
}
